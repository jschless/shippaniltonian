...
selected_nodes = [u for u in selected_nodes if g.nodes[u]
                  ['y'] < 41.03356]  # hard code y coordinate to stop at shippan ave and Ralsey
...
for (v1, v2) in list(mates):
    eulerian_ug.add_edge(
        v1, v2, weight=dist_mat[(v1, v2)], length=dist_mat[(v1, v2)])
...
def plot_route_at_step(g, visited, node_labels=True, edge_labels=True, output_file="temp.png", dist=None, redund_dist=None):
    color_map = {0: "white", 1: "yellow", 2: "red", 3: "orange"}
    ecolors = [color_map[visited.get((u, v,), 0)] for u, v in g.edges()]

    fig, ax = ox.plot_graph(g, edge_color=ecolors,
                            show=False, bgcolor="#7B7A7A", edge_linewidth=1.5)
    ...
    if edge_labels:
        labeled_edges = set()
        for _, edge in edge_df.fillna("").sort_values('length', ascending=False).iterrows():
            text = edge["name"]
    ...
    if dist:
        ax.annotate(f"Total distance: {round(running_dist/1000, 2)} km",
                    (.58, .85), c="white", xycoords="figure fraction")

    if redund_dist:
        ax.annotate(f"Total duplicated: {round(redund_dist/1000, 2)} km",
                    (.58, .83), c="white", xycoords="figure fraction")
...
for i, v in enumerate(new_route[1:]):
    ...
    plot_route_at_step(eulerian_ug, visited, output_file=output_file,
                       edge_labels=False, dist=running_dist, node_labels=False,
...
#     try:
#         lat, lon = ug.nodes[point]['lat'], ug.nodes[point]['lon']
#         gpx_segment.points.append(
#             gpxpy.gpx.GPXTrackPoint(lat, lon, elevation=1))
#     except Exception as e:
#         print(e, f"no lat or lon for {point}: {ug.nodes[point]}")
